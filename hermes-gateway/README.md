...
```
用户浏览器 ──► Hermes Gateway (8000) ──► Hermes Backend (9119)
                    │
                    ├── /login       → 登录页面
...
网关默认监听 `0.0.0.0:8000`，后端地址默认为 `http://127.0.0.1:9119`。
...
| `HERMES_BASE_URL` | Hermes 后端地址 | `http://127.0.0.1:9119` |
...
docker run -d \
  ...
  -e HERMES_BASE_URL=http://host.docker.internal:9119 \
  hermes-gateway
```
